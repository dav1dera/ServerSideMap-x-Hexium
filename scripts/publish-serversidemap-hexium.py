#!/usr/bin/env python3
"""Publish the generated ServerSideMap compatibility package to Hexium."""
...
HEXIUM_PACKAGE = os.environ.get("HEXIUM_PACKAGE", "ServerSideMap_Valheim1Fix")
...
USER_AGENT = "dav1dera-serversidemap-publisher"
...
def package_zip(version: str) -> Path:
    path = DIST / f"{HEXIUM_PACKAGE}-{version}-hexium.zip"
    if not path.is_file() or path.stat().st_size == 0:
        raise SystemExit(f"Hexium package ZIP not found: {path}")
    return path
    ...
    for part in sorted(upload_urls, key=lambda item: int(item["part_number"])):
        ...
        if len(chunk) != length:
            raise RuntimeError(
                ...
                f"but only {len(chunk)} are available"
            )
        request = urllib.request.Request(
            str(part["url"]), data=chunk, headers={"User-Agent": USER_AGENT}, method="PUT"
        )
        try:
...
def main() -> None:
    ...
    if version_already_exists(version):
        print(f"Hexium already has {HEXIUM_TEAM}/{HEXIUM_PACKAGE} {version}; skipping publish")
        return
